...
void main() {
  ...
  vec3 relativePos = (rotated - uCameraPosition + //
                      vec3(1.4, 0, 0) * (InstanceID % 200) +
                      vec3(0, 0, 1.4) * ((InstanceID / 200) % 200) +
                      vec3(0, 1.4, 0) * (InstanceID / 40000)) *
                     uCameraRotation;
...
vec3 rotate(vec3 pos, int InstanceID) {
  float yaw = (InstanceID % 200) * uTime;
  float pitch = ((InstanceID / 200) % 200) * uTime;
  float roll = (InstanceID / 40000) * uTime;
